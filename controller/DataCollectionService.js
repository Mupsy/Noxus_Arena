...
class DataCollectionService {
  ...
  async getTopPlayersForChampion(championId, role, count = 10) {
    try {
      ...
      // Dans une implémentation réelle, vous utiliseriez l'API Riot pour trouver les meilleurs joueurs
      // Ici, nous allons simuler en récupérant des joueurs de haut rang
      
      // 1. Récupérer les joueurs de Challenger/Grandmaster
      const challengerResponse = await axios.get(
        `${this.baseUrl}/lol/league/v4/challengerleagues/by-queue/RANKED_SOLO_5x5`,
        { headers: { "X-Riot-Token": this.riotApiKey } }
      );
      
      // 2. Extraire les summoner IDs
      const summonerIds = challengerResponse.data.entries
        .sort((a, b) => b.leaguePoints - a.leaguePoints)
        .slice(0, 50)
        .map(entry => entry.summonerId);
      
      // 3. Récupérer les données des invocateurs
      const summonersPromises = summonerIds.map(id => 
        axios.get(`${this.baseUrl}/lol/summoner/v4/summoners/${id}`, {
          headers: { "X-Riot-Token": this.riotApiKey }
        })
      );
      ...
      // Trier par winrate
      return topPlayers.sort((a, b) => b.winRate - a.winRate);
  ...
  async getPlayerMatchIds(puuid, count = 20) {
    try {
      const response = await axios.get(
        `${this.matchBaseUrl}/lol/match/v5/matches/by-puuid/${puuid}/ids`,
        {
          params: { count },
          headers: { "X-Riot-Token": this.riotApiKey }
        }
    ...
    try {
      ...
      for (let i = 0; i < matchIds.length; i += batchSize) {
        ...
        const batchPromises = batch.map(matchId => 
          axios.get(`${this.matchBaseUrl}/lol/match/v5/matches/${matchId}`, {
            headers: { "X-Riot-Token": this.riotApiKey }
          }).catch(err => {
        ...
        const batchData = batchResponses
  ...
  async analyzeTopPlayerBuilds(championId, role, playerCount = 5, matchesPerPlayer = 10) {
    try {
      ...
      // 2. Récupérer les parties récentes de ces joueurs avec ce champion
      const allMatches = [];
      
      for (const player of topPlayers) {
        const matchIds = await this.getPlayerMatchIds(player.puuid, matchesPerPlayer * 2); // Récupérer plus pour filtrer
        const matches = await this.getMatchesData(matchIds);
        ...
        const championMatches = matches.filter(match => {
          ...
          return participant !== undefined;
        }).slice(0, matchesPerPlayer); // Limiter au nombre demandé
  ...
  analyzeBuildsFromMatches(matches, championId) {
    try {
      ...
      const participants = matches.map(match => {
  ...
  extractBuildsData(participants) {
    ...
    const mythicItems = [
      6653, 6655, 6656, 6662, 6671, 6672, 6673, 6693, 6694, 
      6675, 6676, 6632, 6664, 6667, 6630, 6631, 3152, 6692
    ];
    
    // Liste des bottes
    const boots = [
      3006, 3047, 3117, 3009, 3158, 3111, 3157
    ];
    ...
    participants.forEach(participant => {
      ...
      const starterItems = [1054, 1055, 1056, 1082, 1083, 1101, 1102, 1103, 1104, 2003, 2033, 3070, 3850, 3854, 3858, 3862];
    ...
    participants.forEach(participant => {
      ...
      if (participant.perks.statPerks) {
        ...
        if (statPerks.offense) {
          statShardsCounter[`offense_${statPerks.offense}`] = (statShardsCounter[`offense_${statPerks.offense}`] || 0) + 1;
        }
        ...
        if (statPerks.defense) {
          statShardsCounter[`defense_${statPerks.defense}`] = (statShardsCounter[`defense_${statPerks.defense}`] || 0) + 1;
        }
      }
    });
