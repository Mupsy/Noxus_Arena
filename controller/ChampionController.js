...
class ChampionController {
  constructor() {
    ...
    this.ddragonVersion = "15.3.1" // Mettre à jour selon la version actuelle
  }
  ...
  async getChampionData(championId) {
    try {
      ...
      const champion = Object.values(champions).find((champ) => champ.key === championId.toString())
  ...
  generateCounters(championId) {
    ...
    const counterIds = [266, 103, 84, 12, 32, 34, 1, 523, 22, 136, 268, 432, 53, 63, 201, 51, 164, 69, 31, 42]

    // Exclure le champion lui-même
    const filteredIds = counterIds.filter((id) => id !== Number.parseInt(championId))
  ...
  generateSynergies(championId) {
    ...
    const synergyIds = [99, 412, 157, 555, 35, 18, 64, 427, 40, 202, 222, 429, 498, 83, 154, 887, 523, 875, 147, 101]

    // Exclure le champion lui-même
    const filteredIds = synergyIds.filter((id) => id !== Number.parseInt(championId))
  ...
  async getRecommendedBuilds(championId, role = "MID") {
    try {
      ...
      if (analyzedData.error || analyzedData.matchesAnalyzed < 1) {
      ...
      return {
        ...analyzedData,
        playstyleTips,
        confidence: Math.min(5 + analyzedData.matchesAnalyzed / 2, 98), // Plus de matchs = plus de confiance, minimum 5%
  ...
  generateBuild(championId, role, style) {
    ...
    const mythic = mythicItems[style] ? mythicItems[style][Math.floor(Math.random() * mythicItems[style].length)] : 6655
  ...
  // Mettre à jour la fonction generateRunes pour utiliser des IDs de runes valides qui correspondent à notre mapping
  ...
  generateRunes(championId, role, style) {
    ...
    const primaryRunes = {
      ...
      8200: [8214, 8229, 8230], // Sorcery
      8300: [8351, 8358, 8360], // Inspiration
      8400: [8437, 8439, 8465], // Resolve
    }
  ...
  generateSummonerSpells(championId, role) {
    ...
    const spells = spellsByRole[role] || [
      [4, 14],
      [4, 6],
      [4, 7],
    ]
